'''recreating the base code to try
and automate the telescope observing requests
...
#     #not sure if I really need this function, but I'm including it just in case
#     #if I include a white True loop this is where I will break it
#     pass


# def pending_status_response():
#     '''should take the pending status and resubmit the observation'''
#     #os.execv(__file__, sys.argv) was recommended as a way to make it cycle through, but might need a while loop
#     # loop = True
#     main()
#     #could maybe stick main() call here? Something to play with


# def window_expired_status_response():
#     '''take expired status and resubmit'''
#     #similar to the pending one
#     # loop = True
#     main()
...
def check_candidate_status(requestgroup):
    ...
    print('waiting')
    time.sleep(3600)#108000) #expires about 30 hours after submission- 108000 seconds
    ...
    completed = -1
    # for i in range(1): #didn't do anything
    if requestgroup['state'] == "COMPLETED":
        completed = 2
        print('complete')
        # exit()
    elif requestgroup['state'] == "PENDING":
        completed = 1  #want this to trigger another submission
        print('pending')
        main('spectroscopic')
        #should I call the response functions here?
    elif requestgroup['state'] == "WINDOW_EXPIRED":
        completed = 0  #want this to trigger another submission
        print('expired')
        main('spectroscopic')
...
def check_observations(API_TOKEN, lco_programs=None):
    ...
    while response_link is not None:
        ...
        #print('circle')
        #for i in range(1):
        # for key, value in requestgroup.items():
        #     if key == 'id':
        #         id_num = value
        #         print(id_num)
